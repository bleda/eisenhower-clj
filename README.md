...
* [`src/clj/eisenhower/`](src/clj/eisenhower/): Backend and middleware source files (Clojure,
[Compojure](https://github.com/weavejester/compojure))
* [`src/cljs/eisenhower/`](src/cljs/eisenhower/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/eisenhower/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/eisenhower/`](test/cljs/eisenhower/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `eisenhower` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/eisenhower/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns eisenhower.example
  (:require [eisenhower.config :as config])
...
```sh
port=2000 java -jar target/eisenhower.jar
```
